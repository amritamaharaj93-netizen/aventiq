    ...
    if (saved) {
      try {
        ...
        if (Array.isArray(list)) {
          const found = list.find((s: any) => s.id === serviceId)
          if (found) {
            setFormData({
              title: found.title || "",
              slug: found.slug || "",
              ...
              techInput: found.tech ? found.tech.join(", ") : "",
              features: found.features && found.features.length > 0 ? found.features : [{ title: "", desc: "" }]
            })
          }
        }
      } catch (e) {
    ...
    if (saved) {
      try {
        ...
        if (Array.isArray(list)) {
          ...
          localStorage.setItem("aventiq_admin_services", JSON.stringify(updatedList))
        }
      } catch (err) {
        console.error("Failed to update service in local storage")
      }
    }
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 max-w-4xl mx-auto pb-12">
      ...
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-extrabold text-[#020B1C] tracking-tight">Edit Service Content</h1>
            <p className="text-[#64748B] text-sm font-medium mt-0.5">Update overview text, tech stack, and deliverable cards for this service.</p>
          </div>
        </div>
      </div>
