...
export default function CreateCareerPage() {
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    const newJob = {
      ...
      description: description || `<p>We are seeking a talented ${role} to join our team.</p>`
    }
    ...
    if (saved) {
      try {
        jobsList = JSON.parse(saved)
      } catch (err) {
        console.error("Failed to parse career postings")
      }
    }

    const updated = Array.isArray(jobsList) ? [...jobsList, newJob] : [newJob]
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 max-w-4xl mx-auto pb-12">
      ...
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-extrabold text-[#020B1C] tracking-tight">Add Career Posting</h1>
            <p className="text-[#64748B] text-sm font-medium mt-0.5">Create a new job opening for applicants.</p>
          </div>
        </div>
      </div>
